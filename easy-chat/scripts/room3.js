...
EasyChat.prototype.initFirebase = function() {
  ...
  var that = this;
  this.firestore.collection('messages3')
    .orderBy('timestamp')
...
EasyChat.prototype.loadMessages = function() {
  // TODO : 08. firestoreから読み込み
  // this.firestore.collection('messages/message')
  //   .orderBy('timestamp')
  //   .get()
  //   .then((querySnapshot) => {
...
EasyChat.prototype.saveMessage = function(e) {
 ...
 //  }
  ...
  if (this.messageInput.value && this.checkSignedInWithMessage()) {
    this.firestore.collection('messages3').add({
...
EasyChat.prototype.displayMessage = function(key, name, text, picUrl) {
  ...
  if (text) {
    // テキスト内のURLをリンクに変換
    var textWithLinks = this.linkify(text);
...
// URLをリンクに変換するメソッド
EasyChat.prototype.linkify = function(text) {
  // URLを検出する正規表現
